// ✅ BADOL V12 - MONGODB 100% + FILE BACKUP SINGLE PATH
const fs = require('fs');
...
async function getLocked(){
  try {
    const cfg = await global.db.getConfig('lockedCommands');
    if(!cfg) return [];
    return Array.isArray(cfg)? cfg : (cfg.list || cfg.data || []);
  } catch { return []; }
}
async function saveLocked(arr){
  try {
    // ✅ MONGODB SAVE
    await global.db.setConfig('lockedCommands', { list: arr, updatedAt: Date.now() });
    // ✅ SINGLE PATH FILE BACKUP: /home/container/config.js
    ...
    try{
      if(fs.existsSync(configPath)){
        ...
        if(raw.trim().startsWith("{")){
          ...
          cfg.settings.lockedCommands = arr;
          fs.writeFileSync(configPath, JSON.stringify(cfg, null, 2), 'utf8');
        } else {
          if(!global.config.settings) global.config.settings = {};
          global.config.settings.lockedCommands = arr;
          fs.writeFileSync(configPath, `module.exports = ${JSON.stringify(global.config, null, 2)};`, 'utf8');
        }
      }
    }catch(e){ console.log("lockfile file save err:", e.message); }
  } catch(e){ console.log("lockfile save err:", e.message); }
}
  ...
  onCallback: async function ({ event, api, ctx }) {
    try {
      ...
      if(data.startsWith("lockfile_toggle_")){
        ...
        const btns = { reply_markup: { inline_keyboard: [[{ text: isLock? `✅ Unlock ${cmdName}` : `🔒 Lock ${cmdName}`, callback_data: `lockfile_toggle_${cmdName}_${page}` }], [{ text: "⬅️ Back", callback_data: `lockfile_page_${page}` }, { text: "❌ Close", callback_data: "lockfile_close" }]] } };
        try { await ctx.editMessageCaption(detail, btns); } catch {}
        return await ctx.answerCbQuery(isLock? `🔒 Locked MongoDB` : `✅ Unlocked`).catch(()=>{});
      }
