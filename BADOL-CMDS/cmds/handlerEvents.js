//✅ BADOL/handlerEvents.js - FULL 540+ LINES - ORIGINAL LOGIC 100% SAME + MONGODB FAST + APPROVAL FIXED + MIGRATION FIXED
...
const GROUP_CMD_FILE = path.join(__dirname, "../data/groupCommands.json");

if (!global._gcmdCache) global._gcmdCache = { data: {}, time: 0 };
if (!global._lockCache) global._lockCache = { data: [], time: 0 };

// ✅ MONGODB + FILE FALLBACK - FAST CACHE
async function getGroupCommands() {
  try {
    if (Date.now() - global._gcmdCache.time < 15000 && Object.keys(global._gcmdCache.data).length>=0) {
      return global._gcmdCache.data;
    }
    if (global.db?.getAllGroupCommands) {
      const dbData = await global.db.getAllGroupCommands();
      if (dbData && Object.keys(dbData).length>=0) {
        global._gcmdCache = { data: dbData, time: Date.now() };
        return dbData;
      }
    }
    if (fs.existsSync(GROUP_CMD_FILE)) {
      const data = JSON.parse(fs.readFileSync(GROUP_CMD_FILE, "utf8"));
      global._gcmdCache = { data, time: Date.now() };
      return data;
    }
    return {};
...
async function isCommandLocked(cmdName, cmdObj = null){
  try {
    let locked = [];
    if (Date.now() - global._lockCache.time < 15000) {
      locked = global._lockCache.data;
    } else if (global.db?.getLockedCommands) {
      const list = await global.db.getLockedCommands();
      locked = list.map(c=>c.toLowerCase());
      global._lockCache = { data: locked, time: Date.now() };
    } else if(fs.existsSync(LOCK_FILE)) {
      locked = JSON.parse(fs.readFileSync(LOCK_FILE, "utf8")).map(c=>c.toLowerCase());
      global._lockCache = { data: locked, time: Date.now() };
    } else return false;
    ...
    if(cmdObj){
      ...
      if(locked.includes(mainName) && cmdObj.config.aliases?.map(a=>a.toLowerCase()).includes(low)) return true;
    }
    if(global.badol && global.badol.commands){
      const directCmd = global.badol.commands.get(low);
      if(directCmd){
        const main = directCmd.config.name.toLowerCase();
        if(locked.includes(main)) return true;
      }
      for(const [_, c] of global.badol.commands){
        if(c.config.aliases && c.config.aliases.map(a=>a.toLowerCase()).includes(low)){
          if(locked.includes(c.config.name.toLowerCase())) return true;
        }
        if(c.config.name.toLowerCase() === low && c.config.aliases){
          for(const al of c.config.aliases){
            if(locked.includes(c.config.name.toLowerCase())) return true;
          }
        }
      }
...
(function loadAdminOnlyOnStartup(){
  try {
    if (fs.existsSync(ADMINONLY_FILE)) {
      ...
      console.log(`[Eren-AI] AdminOnly Loaded: ${data.enabled? "ON 🔒" : "OFF 🔓"}`);
    }
  } catch (e) {}
})();
...
function getSettings() {
  ...
  } catch { adminOnlyMode = st.adminOnlyMode === true || st.onlyAdmin === true; }
...
async function sendBotStartNotification(api) {
  try {
    ...
    const line = '━━━━━━━━━━━━━━━━━━━━━━━━';
    const botName = getConfig('botName', 'Eren-AI');
    const ownerName = getConfig('ownerName', 'B4D9L & M9U');
...
async function handleMessage(ctx) {
  try {
    ...
    // ✅✅✅ MIGRATION FIX - GROUP ID CHANGE HANDLE - DUPLICATE FIX
    try{
    ...
    if (firstWord.startsWith('/') && firstWord.includes('@')) {
      ...
      const myBotFromConfig = (global.botUsername || global.config?.botInfo?.username || ctx.botInfo?.username || "ErenAi1Bot").toLowerCase().replace('@','');
      ...
      if (command && command.config.name!== "refer" &&!isOwner) {
        try {
          ...
          if (isPaid) {
            ...
            if (!check.ok) {
              const botUser = global.botUsername || ctx.botInfo?.username || "ErenAi1Bot";
...
📌 Must Join: @erenaiteam`,
      ...
      if ((msg.chat.type === 'group' || msg.chat.type === 'supergroup') && command) {
        if (commandName!== "gcmd" &&!(await isGroupCommandAllowed(chatId, commandName))) return;
      }
      if (command) {
        // ✅✅✅ MAIN FIX - MUST AWAIT - 100% APPROVAL NOTICE
    ...
    if (data.startsWith('request_approve_') || data.startsWith('request_reject_') || data.startsWith('approve_unban_') || data.startsWith('reject_unban_') || data.startsWith('approve_group_') || data.startsWith('reject_group_') || data.startsWith('approve_dm_') || data.startsWith('reject_dm_')) {
      if (data.startsWith('request_approve_') || data.startsWith('request_reject_') || data.startsWith('approve_unban_') || data.startsWith('reject_unban_')) {
        ...
        try {
          ...
          let all = {};
          if(fs.existsSync(approvalFile)){ try{ all = JSON.parse(fs.readFileSync(approvalFile, "utf8")); }catch{} }
...
async function handleNewMember(ctx) { try { const st = getSettings(); if (!st.welcome) return; const msg = ctx.message || ctx.update?.message || ctx.update; const newMembers = msg?.new_chat_members || ctx.new_chat_members; if (!newMembers?.length) return; const message = new MessageUtils(ctx); if (global.badol.events) { for (const [_, event] of global.badol.events) { if (event.config.eventType === 'new_member' && event.BADOL) { try { if (msg.chat?.id &&!(await isGroupCommandAllowed(msg.chat.id, event.config.name))) continue; await event.BADOL.call(event, { event: msg, api: ctx.telegram, message, newMembers, ctx }); } catch {} } } } } catch (e) {} }
async function handleLeftMember(ctx) { try { const st = getSettings(); if (!st.leave) return; const update = ctx.update || {}; const msg = ctx.message || update.message || update.edited_message || {}; let leftMember = msg.left_chat_member || ctx.left_chat_member; if (!leftMember && update.chat_member) { const oldStatus = update.chat_member.old_chat_member?.status; const newStatus = update.chat_member.new_chat_member?.status; if (['left', 'kicked'].includes(newStatus) && ['member', 'administrator', 'restricted'].includes(oldStatus)) leftMember = update.chat_member.new_chat_member.user; } if (!leftMember) return; const message = new MessageUtils(ctx); if (global.badol.events) { for (const [_, event] of global.badol.events) { if (event.config.eventType === 'left_member' && event.BADOL) { try { if (msg.chat?.id &&!(await isGroupCommandAllowed(msg.chat.id, event.config.name))) continue; await event.BADOL.call(event, { event: Object.keys(msg).length > 0? msg : update.chat_member || ctx, api: ctx.telegram, message, leftMember, ctx }); } catch {} } } } } catch (e) {} }
async function handleReaction(ctx) { try { const reaction = ctx.messageReaction; if (!reaction) return; const st = getSettings(); const messageId = reaction.message_id; const chatId = reaction.chat.id; const userId = reaction.user.id; if (st.adminUnsend) { const adminUID = getConfig('adminUID', []); const isAdmin = adminUID.includes(String(userId)); if (isAdmin && reaction.new_reaction?.length > 0) { const reactionEmojis = reaction.new_reaction.filter(r => r.type === 'emoji').map(r => r.emoji); if (reactionEmojis.includes(st.adminUnsendEmoji)) { try { await ctx.telegram.deleteMessage(chatId, messageId); return; } catch {} } } } if (global.badol.events) { for (const [_, event] of global.badol.events) { if (event.config.eventType === 'reaction' && event.BADOL) { try { await event.BADOL.call(event, { event: reaction, api: ctx.telegram, ctx }); } catch {} } } } } catch (error) {} }
