...
module.exports = {
    ...
    try {
      if (!args[0] &&!event.reply_to_message) {
        return message.reply(
          "📢 **Noti V13 MongoDB Guide:**\n\n" +
          "• /noti all <msg> - সব গ্রুপ+ইউজারে\n" +
          "• /noti <msg> - ফটোতে reply দিয়ে (ফটো সহ)\n" +
          "• /noti list - গ্রুপ লিস্ট (Real Atlas)\n" +
          "• /noti dm - ইউজার লিস্ট (Real Atlas)\n" +
      ...
      if (sub === 'list') {
        ...
        if (!groups.length) return message.reply('❌ কোনো গ্রুপ নাই - MongoDB খালি');
        let txt = `📋 All Groups MongoDB (${groups.length}):\n\n`;
        groups.forEach((g, i) => {
      ...
      if (sub === 'dm') {
        ...
        if (!users.length) return message.reply('❌ কোনো DM ইউজার নাই - MongoDB খালি');
        let txt = `📋 DM Users MongoDB (${users.length}):\n\n`;
  ...
  sendBroadcast: async function (api, message, event, notiMessage, mode = 'all', targets = []) {
    let statusMsg = null;
    try { statusMsg = await message.reply('⏳ Preparing broadcast MongoDB...'); } catch {}

    const header = "⚜️ Eren-AI Official Broadcast ⚜️\n━━━━━━━━━━━━━━━━━━━━\n\n";
    const footer = "\n\n━━━━━━━━━━━━━━━━━━━━\n🤖 Powered by Eren-AI | MongoDB";
        ...
        if (em.includes("not found") || em.includes("kicked") || em.includes("blocked") || em.includes("forbidden") || em.includes("deactivated") || em.includes("chat not found")) {
          try {
            ...
            if (String(chatId).startsWith("-")) {
              await global.db.deleteThread(String(chatId));
            } else {
              await global.db.deleteUser(String(chatId));
            }
            removed++;
          } catch {}
        }
